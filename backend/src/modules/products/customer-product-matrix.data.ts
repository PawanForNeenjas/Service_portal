...
// Imported from:
// C:\Users\Jiya\Downloads\Product List for CRM- Customer Wise.xlsx
// Notes:
// - 188 non-header rows were read from the sheet.
// - 3 exact duplicate configuration rows were removed.
// - 1 malformed row (missing rating) was skipped.
const seedRows: CustomerProductSeedRow[] = [
...
export const customerProductMatrix: CustomerProductConfiguration[] = seedRows.map((row) => {
...
export function findCustomerProductConfigurationById(id: string) {
  return customerProductMatrix.find((configuration) => configuration.id === id);
}

function buildConfigurationId(row: CustomerProductSeedRow) {
  return `cfg-${slugify(row.customerName)}-${slugify(row.volt)}-${slugify(row.amp)}-${slugify(row.rating)}`;
}

function buildBrandCode(customerName: string) {
...
function buildModelCode(volt: string, amp: string, rating: string) {
...
function buildInternalReference(row: CustomerProductSeedRow) {
