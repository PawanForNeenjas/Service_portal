...
export default function PartnerTickets() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Tickets"
        description="View dealer tickets."
        actions={
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            <CardTitle>Dealer Tickets</CardTitle>
            <p className="mt-1 text-sm text-slate-500">Current dealer tickets.</p>
          </div>
        ...
        <CardContent>
                ...
                {tickets.map((ticket) => (
                  <Link key={ticket.id} to={`/tickets/${ticket.id}`} className="block rounded-lg border bg-white p-4">
                    ...
                    <div className="mt-4 flex items-center justify-between text-sm">
                      <span className="text-slate-500">{ticket.customer?.name ?? "Customer unavailable"}</span>
          ...
          {!loading && !tickets.length ? (
            <div className="rounded-lg border border-dashed bg-slate-50 p-6 text-center">
              ...
              <p className="mt-3 text-sm font-semibold text-slate-950">No tickets available</p>
              <p className="mt-1 text-sm text-slate-500">
